            ...
            <div v-if="configForm.timeoutEnable">
              ...
              >
                <el-date-picker
                  ...
                  type="datetime"
                  placeholder="Please select 日期和时间"
...
const formRules = reactive({
  async: [{ required: true, message: '是否异步 can not empty', trigger: 'change' }],
  calledProcessDefinitionKey: [{ required: true, message: '子流程 can not empty', trigger: 'change' }],
  skipStartUserNode: [
    { required: true, message: '是否自动跳过子流程发起节点 can not empty', trigger: 'change' }
  ],
  startUserType: [{ required: true, message: '子流程发起人 can not empty', trigger: 'change' }],
  startUserEmptyType: [
    { required: true, message: '当子流程发起人为空时 can not empty', trigger: 'change' }
  ],
  startUserFormField: [{ required: true, message: '发起人表单 can not empty', trigger: 'change' }],
  timeoutEnable: [{ required: true, message: '超时设置是否开启 can not empty', trigger: 'change' }],
  timeoutType: [{ required: true, message: '超时设置时间 can not empty', trigger: 'change' }],
  timeDuration: [{ required: true, message: '超时设置时间 can not empty', trigger: 'change' }],
  dateTime: [{ required: true, message: '超时设置时间 can not empty', trigger: 'change' }],
  multiInstanceEnable: [{ required: true, message: '多实例设置 can not empty', trigger: 'change' }]
})

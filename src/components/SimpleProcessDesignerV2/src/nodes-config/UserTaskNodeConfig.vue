...
const formRules = reactive({
  candidateStrategy: [{ required: true, message: '审批人设置 can not empty', trigger: 'change' }],
  userIds: [{ required: true, message: '用户 can not empty', trigger: 'change' }],
  roleIds: [{ required: true, message: 'Role can not empty', trigger: 'change' }],
  deptIds: [{ required: true, message: 'Department can not empty', trigger: 'change' }],
  userGroups: [{ required: true, message: '用户组 can not empty', trigger: 'change' }],
  formUser: [{ required: true, message: '表单内用户字段 can not empty', trigger: 'change' }],
  formDept: [{ required: true, message: '表单内部门字段 can not empty', trigger: 'change' }],
  postIds: [{ required: true, message: 'Position can not empty', trigger: 'change' }],
  expression: [{ required: true, message: '流程表达式 can not empty', trigger: 'blur' }],
  approveMethod: [{ required: true, message: '多人审批方式 can not empty', trigger: 'change' }],
  approveRatio: [{ required: true, message: '通过比例 can not empty', trigger: 'blur' }],
  returnNodeId: [{ required: true, message: '驳回节点 can not empty', trigger: 'change' }],
  timeoutHandlerEnable: [{ required: true }],
  timeoutHandlerType: [{ required: true }],
  timeDuration: [{ required: true, message: '超时时间 can not empty', trigger: 'blur' }],
  maxRemindCount: [{ required: true, message: '提醒次数 can not empty', trigger: 'blur' }],
  assignEmptyHandlerType: [{ required: true }],
  assignEmptyHandlerUserIds: [{ required: true, message: '用户 can not empty', trigger: 'change' }],

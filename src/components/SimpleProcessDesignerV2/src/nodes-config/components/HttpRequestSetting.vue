<template>
  ...
  <el-form-item
    ...
    :rules="{
      required: true,
      message: 'Request address can not empty',
      trigger: 'blur'
    }"
  >
      ...
      <div class="flex pt-2" v-for="(item, index) in setting.response" :key="index">
        <div class="mr-2">
          <el-form-item
            ...
            :rules="{
              required: true,
              message: '表单字段 can not empty',
              trigger: 'blur'
            }"
          >
            <el-select class="w-160px!" v-model="item.key" placeholder="Please select Form column">
              <el-option

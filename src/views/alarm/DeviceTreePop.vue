...
const checkNode = () => {
  ...
  const nodes = checkedNodes.filter((node) => node.type === 'device')
  console.log("node ====== ",nodes)
  for (let i = 0; i < nodes.length; i++) {
    if (props.cameraType == '3') {
      nodeData.value.push({name: nodes[i].name, value : nodes[i].platformCameraId})
    } else if (props.cameraType == '4') {
      nodeData.value.push({name: nodes[i].name,value: nodes[i].id})
    }
  }
}
...
<template>
  ...
  >
    <template #default="{ node, data }">
      <span class="tree-node">
        <el-icon v-if="data.type === 'device'" class="tree-icon" color="skyblue">
          <VideoCamera />
        </el-icon>
  ...
  <div style="display: flex; flex-direction: row">
    ...
    <el-button class="vms-cancel-button" @click="cancelSelect" size="default"> Cancel </el-button>
  </div>
</template>

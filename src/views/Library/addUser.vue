    ...
    >
      <el-tab-pane :key="1" label="Detail" name="Detail" v-if="tabTool.showDetailTabFlag">
        <div style="position: relative">
          <div
            style="position: absolute; right: -10px; top: -20px; z-index: 9999"
            v-show="props.libraryType == 2"
          >
            <h1>
              <el-link
                type="primary"
            ...
            <el-row :gutter="20">
              ...
              <el-col :span="18">
                ...
                >
                  ...
                  <el-descriptions-item
                    label="Reason For Surveillance"
                    label-width="180"
                    v-if="props.libraryType == 2"
                    label-align="right"
                  >
                    {{ formData.surveillanceReason }}
                  </el-descriptions-item>
                  <el-descriptions-item label="Nationality" label-align="right">
                    {{ formData.remark }}
                  </el-descriptions-item>
                  <el-descriptions-item
                    label="Alarm Contact Person"
                    label-width="180"
                    v-if="props.libraryType == 2"
                  >
                    {{ formData.alarmContactPerson }}
                  </el-descriptions-item>
                </el-descriptions>
              </el-col>
            </el-row>
              ...
              <div style="height: 100%; overflow-y: auto">
                ...
                >
                  <el-input
                    type="textarea"
                    :rows="4"
                    v-model="formData.surveillanceReason"
                    placeholder=""
                    :maxlength="-1"
                ...
                >
                  <el-input
                    type="textarea"
                    :rows="2"
                    v-model="formData.alarmContactPerson"
                    placeholder=""
                    :maxlength="-1"
                    :show-word-limit="false"
                    :autosize="{ minRows: 2, maxRows: 4 }"
...
import Detail from '../vehicle/components/detail.vue'
import LibraryForm from './manage/LibraryForm.vue'
...
const getOne = async () => {
  ...
  if (props.libraryType == 1) {
    const data = {
      platformPersonId: platformPersonId,
      albumId: props.albumId
    }
    const result = await PersonApi.get2(data)
    formData.value = result.data
    console.log('formData.value ==== ', formData.value)
    imageInfo.value.defaultImage = imageInfo.value.djBaseUrl + formData.value.url
  } else {
    formData.value = await PersonApi.get(platformPersonId)
    imageInfo.value.defaultImage = getKsProxyUrl(formData.value.url)
  }
}
...
const previewImage = () => {
  imageInfo.value.previewList = []
  imageInfo.value.previewList.push(imageInfo.value.defaultImage)
}

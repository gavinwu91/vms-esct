...
defineOptions({name: 'FaceTracking'})
...
const changeMap = () => {
  ...
  if (mapTools.value.mapType) {
    mapTools.value.mapType = false;
    map.getLayers().item(0).setVisible(true);
    map.getLayers().item(1).setVisible(false);
  } else {
    mapTools.value.mapType = true;
...
<template>
  <div class="track_map_main" style=" width: 100%;height: 800px;" v-loading="loading">
    ...
    <div class="maptool earthicon " :class="{'active':mapTools.mapType}" @click="changeMap">
      <img :src="getImageUrl('earth.png')"/>
    </div>

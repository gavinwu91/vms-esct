<template>
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button type="info" plain @click="handleJobLog()" v-hasPermi="['infra:job:query']">
          <Icon icon="ep:zoom-in" class="mr-5px" /> Execute log
        </el-button>
      </el-form-item>
    </el-form>
  </ContentWrap>
  ...
  <ContentWrap>
    <el-table v-loading="loading" :data="list">
      ...
      <el-table-column label="Task Name" align="center" prop="name" />
      ...
      <el-table-column label="Process " align="center" prop="handlerParam" />
      <el-table-column label="CRON" align="center" prop="cronExpression" />
      <el-table-column label="Operate" align="center" width="200">
        <template #default="scope">
          ...
          <el-button
            ...
            v-hasPermi="['infra:job:update']"
          >
            {{ scope.row.status === InfraJobStatusEnum.STOP ? 'Enable' : 'Pause' }}
          </el-button>
          <el-button
            type="danger"
            link
...
const handleExport = async () => {
  try {
    ...
    download.excel(data, 'Cron job.xls')
  } catch {
  } finally {
...
const handleChangeStatus = async (row: JobApi.JobVO) => {
  try {
    // 修改 Status的二次确认
    const text = row.status === InfraJobStatusEnum.STOP ? 'Enable' : 'Disable'
    await message.confirm(
      'Sure ' + text + ' id is "' + row.id + '" data?',
      t('common.reminder')
    )
    const status =
...
const handleRun = async (row: JobApi.JobVO) => {
  try {
    // 二次确认
    await message.confirm('Sure immediatly execute onece ' + row.name + '?', t('common.reminder'))
    // 提交执行

  ...
  <div class="flex">
    ...
    <el-card :gutter="12" class="w-1/2" shadow="always">
      ...
      <div class="flex items-center">
        <span class="mr-4 text-lg font-medium"> 连接 Status: </span>
      ...
      <el-input
        ...
        :disabled="!getIsOpen"
        clearable
        placeholder="Please input 你要发送的消息"
        type="textarea"
      />
...
watchEffect(() => {
  ...
  try {
    ...
    // 2.1 解析 type Message type
    const jsonMessage = JSON.parse(data.value)
    const type = jsonMessage.type
    const content = JSON.parse(jsonMessage.content)
    if (!type) {
      message.error('未知的Message type：' + data.value)
      return
    }
    ...
    if (type === 'demo-message-receive') {
      ...
      if (single) {
        messageList.value.push({
          text: `【单发】User id(${content.fromUserId})：${content.text}`,
          time: new Date().getTime()
        })
      } else {
        messageList.value.push({
          text: `【群发】User id(${content.fromUserId})：${content.text}`,
          time: new Date().getTime()
        })
      }
      return

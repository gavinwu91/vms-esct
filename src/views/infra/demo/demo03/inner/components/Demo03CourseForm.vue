      ...
      <el-table-column label="名字" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.name`" :rules="formRules.name" class="mb-0px!">
            <el-input v-model="row.name" placeholder="Please input 名字" />
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column label="分数" min-width="150">
        <template #default="{ row, $index }">
          <el-form-item :prop="`${$index}.score`" :rules="formRules.score" class="mb-0px!">
            <el-input v-model="row.score" placeholder="Please input 分数" />
          </el-form-item>
        </template>
      </el-table-column>
      <el-table-column align="center" fixed="right" label="Operate" width="60">
...
const formRules = reactive({
  studentId: [{ required: true, message: '学生编号 can not empty', trigger: 'blur' }],
  name: [{ required: true, message: '名字 can not empty', trigger: 'blur' }],
  score: [{ required: true, message: '分数 can not empty', trigger: 'blur' }]
})

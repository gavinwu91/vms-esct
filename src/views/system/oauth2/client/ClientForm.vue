<template>
  <Dialog v-model="dialogVisible" :title="dialogTitle" max-height="500px" scroll>
    <el-form
      ...
      label-width="160px"
    >
      <el-form-item label="Client Code" prop="secret">
        <el-input v-model="formData.clientId" placeholder="Please input Client Code" />
      </el-form-item>
      <el-form-item label="Client Secret" prop="secret">
        <el-input v-model="formData.secret" placeholder="Please input Client Secret" />
      </el-form-item>
      ...
      <el-form-item label="应用描述">
        <el-input v-model="formData.description" placeholder="Please input Name" type="textarea" />
      </el-form-item>
      ...
      <el-form-item label="授权 Type" prop="authorizedGrantTypes">
        <el-select
          v-model="formData.authorizedGrantTypes"
          filterable
          multiple
          placeholder="Please input 授权 Type"
          style="width: 500px"
        >
          <el-option
      ...
      <el-form-item label="可重定向的 URI 地址" prop="redirectUris">
        <el-select
          ...
          filterable
          multiple
          placeholder="Please input 可重定向的 URI 地址"
          style="width: 500px"
        >
          <el-option
      ...
      <el-form-item label="Additional information" prop="additionalInformation">
        <el-input
          ...
          placeholder="Please input Additional information，JSON Format Data"
          type="textarea"
        />
      </el-form-item>
    </el-form>
...
const formRules = reactive({
  ...
  secret: [{ required: true, message: 'Client Secret can not empty', trigger: 'blur' }],
  name: [{ required: true, message: 'Name can not empty', trigger: 'blur' }],
  logo: [{ required: true, message: '应用图标 can not empty', trigger: 'blur' }],
  status: [{ required: true, message: 'Status can not empty', trigger: 'blur' }],
  accessTokenValiditySeconds: [
    { required: true, message: '访问令牌的有效期 can not empty', trigger: 'blur' }
  ],
  refreshTokenValiditySeconds: [
    { required: true, message: '刷新令牌的有效期 can not empty', trigger: 'blur' }
  ],
  redirectUris: [{ required: true, message: '可重定向的 URI 地址 can not empty', trigger: 'blur' }],
  authorizedGrantTypes: [{ required: true, message: '授权 type can not empty', trigger: 'blur' }]
})

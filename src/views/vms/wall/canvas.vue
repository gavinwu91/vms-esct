...
const initWebRTCStream = async (index, camera) => {
       console.log("destroyVideoStream =========== 123");
  ...
  pc.onicecandidate = event => {
    if (!event.candidate) {
      const url = `http://localhost:8000/v0/cameras/${camera.id}/streams/1/sdp`;
      axios.post(url, pc.localDescription.sdp, {
...
const renderVideos = () => {
  ...
  // 缩放模式 - 只渲染一个视频
  if (zoomedIndex.value !== null) {
    const cell = gridCells.value[zoomedIndex.value];
    if (cell?.camera) {
    ...
    if (video && video.readyState >= video.HAVE_ENOUGH_DATA) {
      try {
        ...
        if (videoAspect > cellAspect) {
          ...
          renderWidth = cellHeight * videoAspect;
          offsetX = x + (cellWidth - renderWidth) / 2;
          offsetY = y;
        } else {
          // 视频更高
          renderWidth = cellWidth;
          renderHeight = cellWidth / videoAspect;
          offsetX = x;
          offsetY = y + (cellHeight - renderHeight) / 2;
        }
...
// 性能管理
let lastDrawTime = 0, frameCount = 0, lastFpsUpdate = 0;

        ...
        >
          ...
          <el-form-item :label="`${nodeTypeName}意见`" prop="reason">
            <el-input
              ...
              :placeholder="`Please input${nodeTypeName}意见`"
              type="textarea"
              :rows="4"
            />
          </el-form-item>
          <el-form-item
            label="Approval Person of the Next Node"
            prop="nextAssignees"
...
const approveReasonRule = computed(() => {
  return {
    reason: [
      { required: reasonRequire.value, message: nodeTypeName + '意见 can not empty', trigger: 'blur' }
    ],
    signPicUrl: [{ required: true, message: '签名 can not empty', trigger: 'change' }],
    nextAssignees: [{ required: true, message: '审批人 can not empty', trigger: 'blur' }]
  }
})
...
const rejectReasonRule = computed(() => {
  return {
    reason: [{ required: reasonRequire.value, message: '审批意见 can not empty', trigger: 'blur' }]
  }
})
...
const copyFormRule = reactive<FormRules<typeof copyForm>>({
  copyUserIds: [{ required: true, message: '抄送人 can not empty', trigger: 'change' }]
})
...
const transferFormRule = reactive<FormRules<typeof transferForm>>({
  assigneeUserId: [{ required: true, message: '新审批人 can not empty', trigger: 'change' }],
  reason: [{ required: true, message: '审批意见 can not empty', trigger: 'blur' }]
})
...
const delegateFormRule = reactive<FormRules<typeof delegateForm>>({
  delegateUserId: [{ required: true, message: '接收人 can not empty', trigger: 'change' }],
  reason: [{ required: true, message: '审批意见 can not empty', trigger: 'blur' }]
})
...
const addSignFormRule = reactive<FormRules<typeof addSignForm>>({
  addSignUserIds: [{ required: true, message: '加签处理人 can not empty', trigger: 'change' }],
  reason: [{ required: true, message: '审批意见 can not empty', trigger: 'blur' }]
})
...
const deleteSignFormRule = reactive<FormRules<typeof deleteSignForm>>({
  deleteSignTaskId: [{ required: true, message: '减签人员 can not empty', trigger: 'change' }],
  reason: [{ required: true, message: '审批意见 can not empty', trigger: 'blur' }]
})
...
const returnFormRule = reactive<FormRules<typeof returnForm>>({
  targetTaskDefinitionKey: [{ required: true, message: '退回节点 can not empty', trigger: 'change' }],
  returnReason: [{ required: true, message: '退回理由 can not empty', trigger: 'blur' }]
})
...
const cancelFormRule = reactive<FormRules<typeof cancelForm>>({
  cancelReason: [{ required: true, message: '取消理由 can not empty', trigger: 'blur' }]
})
...
const handleAudit = async (pass: boolean, formRef: FormInstance | undefined) => {
  ...
  try {
    ...
    if (pass) {
      ...
      // TODO vms 任务有多表单这里要如何处理，会和可编辑的字段冲突

  ...
  <Dialog v-model="dialogVisible" title="保存表单" width="600">
    <el-form ref="formRef" :model="formData" :rules="formRules" label-width="80px">
      <el-form-item label="表单名" prop="name">
        <el-input v-model="formData.name" placeholder="Please input 表单名" />
      </el-form-item>
      ...
      <el-form-item label="Remark" prop="remark">
        <el-input v-model="formData.remark" placeholder="Please input 备注" type="textarea" />
      </el-form-item>
    </el-form>
    <template #footer>
...
const designerConfig = ref({
  ...
  useTemplate: false, // 是否生成vue2语法的Template 组件
  formOptions: {
    form: {
...
const formRules = reactive({
  name: [{ required: true, message: '表单名 can not empty', trigger: 'blur' }],
  status: [{ required: true, message: '开启 Status can not empty', trigger: 'blur' }]
})

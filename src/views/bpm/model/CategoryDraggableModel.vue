  ...
  <el-collapse-transition>
    <div v-show="isExpand">
      ...
      >
        ...
        <el-table-column label="流程 Type" prop="type" min-width="120">
          <template #default="{ row }">
  ...
  <Dialog :fullscreen="false" class="rename-dialog" v-model="renameCategoryVisible" width="400">
    ...
    <template #footer>
      <div class="pr-25px pb-25px">
        <el-button @click="renameCategoryVisible = false">Cancel</el-button>
...
const isModelSorting = ref(false) // 是否正处于排序 Status
const originalData = ref<ModelInfo[]>([]) // 原始数据
const modelList = ref<ModelInfo[]>([]) // 模型列表
const isExpand = ref(false) // 是否处于展开 Status
...
const handleChangeState = async (row: any) => {
  ...
  try {
    ...
    message.success(statusState + ' success')
    // 刷新列表
    emit('success')
  } catch {}
}
